...
describe('isDuplicateJobError', () => {
  it('detects BullMQ custom jobId collisions', () => {
    expect(isDuplicateJobError(new Error('Job msg-1 already exists'))).toBe(
      true
    );
...
describe('enqueue helpers', () => {
  it('returns false when Redis is not configured', async () => {
    ...
    await expect(
      enqueueAiConversationAnalyze({
        ...
        runId: 'run-1',
        idempotencyKey: 'acc-1:conv-1:message:msg-1',
      })
    ).resolves.toBe(false);
    await expect(
      enqueueAiSalesPatternDiscover({
  ...
  it('adds a conversation-analyze job keyed by the idempotency key', async () => {
    ...
    expect(add).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        jobId: 'run-1',
        attempts: 5,
        deduplication: expect.objectContaining({ id: 'acc-1:conv-1' }),
      })
    );
  });
  ...
  it('adds a sales-pattern-discover job keyed by accountId:patterns', async () => {
    ...
    expect(add).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        jobId: 'acc-1--patterns--run-pattern-1',
        attempts: 5,
        deduplication: expect.objectContaining({ id: 'acc-1' }),
      })
    );
  });

  it('coalesces recommendation-intelligence jobs by account', async () => {
    mockGetBullmqConnection.mockReturnValue({
      host: '127.0.0.1',
      port: 6379,
    } as never);
    await expect(
      enqueueAiRecommendationIntelligence({
        accountId: 'acc-1',
        runId: 'run-recommend-1',
        idempotencyKey: 'acc-1:recommendation-intelligence',
      })
    ).resolves.toBe(true);
    expect(add).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ accountId: 'acc-1' }),
      expect.objectContaining({
        jobId: 'acc-1--recommendation-intelligence--run-recommend-1',
        deduplication: expect.objectContaining({
          id: 'acc-1:recommendation-intelligence',
        }),
      })
    );
  });
  ...
  it('adds a sales-pattern-effectiveness job keyed by accountId:effectiveness', async () => {
    ...
    expect(add).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ accountId: 'acc-1' }),
      expect.objectContaining({
        jobId: 'acc-1--effectiveness--run-effectiveness-1',
        attempts: 5,
      })
    );
  });

  it('adds a behavior-optimization job keyed by accountId:ai-behavior-optimization', async () => {
    mockGetBullmqConnection.mockReturnValue({
      host: '127.0.0.1',
      port: 6379,
    } as never);
    await expect(
      enqueueAiBehaviorOptimization({
        accountId: 'acc-1',
        runId: 'run-optimization-1',
        idempotencyKey: 'acc-1:ai-behavior-optimization',
      })
    ).resolves.toBe(true);
    expect(add).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ accountId: 'acc-1' }),
      expect.objectContaining({
        jobId: 'acc-1--ai-behavior-optimization--run-optimization-1',
        attempts: 5,
      })
    );
  });
