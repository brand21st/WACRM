...
import { catalogAnalyticsSince, loadCatalogAnalyticsDashboard, parseCatalogAnalyticsRange } from './aggregate'
...
describe('catalog analytics events', () => {
  it('does not write when the flag is off', async () => {
    const db = createCatalogMemoryDb({
      ...
      catalog_products: [{ id: 'p1', account_id: 'acct-a', handle: 'saree', title: 'Saree', status: 'active' }],
    })
  ...
  it('stores only real catalog ids and skips invented titles', async () => {
    const db = createCatalogMemoryDb({
      ...
      catalog_products: [
        { id: 'p1', account_id: 'acct-a', handle: 'saree', title: 'Black Wedding Saree', status: 'active' },
      ],
    })
    await recordCatalogProductEvents(db, {
      accountId: 'acct-a',
    ...
    const { data } = await db.from('catalog_product_events').select()
    expect(data).toEqual([
      expect.objectContaining({ product_id: 'p1', event: 'search_match', account_id: 'acct-a' }),
    ])
  })
  ...
  it('does not leak another account product id', async () => {
    const db = createCatalogMemoryDb({
      ai_configs: [{ account_id: 'acct-a', catalog_analytics: 'on' }],
      catalog_products: [
        { id: 'p-b', account_id: 'acct-b', handle: 'other', title: 'Other', status: 'active' },
      ],
    })
    await recordCatalogProductEvents(db, {
      accountId: 'acct-a',
  ...
  it('attributes add-to-cart and purchase through retailer_id', async () => {
    const db = createCatalogMemoryDb({
      ai_configs: [{ account_id: 'acct-a', catalog_analytics: 'on' }],
      catalog_products: [
        { id: 'p1', account_id: 'acct-a', handle: 'saree', title: 'Saree', status: 'active' },
      ],
      catalog_variants: [
        {
          id: 'v1',
...
describe('catalog analytics aggregation', () => {
  it('parses ranges and groups counts', async () => {
    ...
    expect(catalogAnalyticsSince('today', now).toISOString()).toBe('2026-09-08T00:00:00.000Z')
    ...
    const db = createCatalogMemoryDb({
      ...
      catalog_variants: [
        { id: 'v1', account_id: 'acct-a', product_id: 'p1', available: false, inventory_quantity: 0 },
      ],
