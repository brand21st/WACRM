...
export type CatalogAnalyticsSource =
  'search_products' | 'get_product' | 'card_send' | 'whatsapp_order';
...
export async function recordCatalogProductEvents(
  ...
  try {
    ...
    const payload = ids.map((productId) => ({
      ...
      quantity,
      source: args.source,
      idempotency_key: args.sourceEventId
        ? [
            args.source,
            args.sourceEventId,
            args.event,
    ...
    const query = db.from('catalog_product_events');
    const { error } = args.sourceEventId
    ...
    if (error) throw error;
    if (args.event === 'add_to_cart' || args.event === 'purchase') {
      void enqueueAiRecommendationIntelligence(
        aiRecommendationIntelligenceJob(args.accountId)
      );
    }
  } catch (err) {
...
export async function recordCatalogLineEvents(
  ...
  try {
    ...
    for (const line of args.lines) {
      const retailerId =
        typeof line.retailer_id === 'string' ? line.retailer_id.trim() : '';
      if (!retailerId) continue;
      const product = await getProductByRetailerId(
        db,
        args.accountId,
        retailerId
      );
