...
/**
 * Inbox list order: newest activity first. Uses `last_message_at`,
 * falling back to `created_at` for empty threads. Invalid dates
 * sort as oldest.
 */
function inboxRecencyMs(conversation: Conversation): number {
...
export function sortInboxConversations(
  ...
  return [...conversations].sort(
    (a, b) => inboxRecencyMs(b) - inboxRecencyMs(a),
  );
}
