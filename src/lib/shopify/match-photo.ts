 ...
 * photo. Searches snapshot + live Shopify (union), then ranks. Optionally
 * vision-confirms against catalog listing images. Returns 0–3 hits that
 * actually score against the description — never invents products.
 */
export async function matchProductsFromPhoto(
  ...
  const queries = photoMatchQueries(desc)
  await Promise.all(
    queries.map(async (q) => {
      try {
  ...
  const confirmed = await maybeConfirmCatalogImages(pool, opts)
  if (confirmed) {
    return confirmed
 ...
 * almost never ILIKE-matches a catalog title.
 */
...
async function maybeConfirmCatalogImages(
  ...
  const withImages = pool.filter((p) => p.imageUrl)
  if (withImages.length < 2 && !opts.confirmImpl) return null
