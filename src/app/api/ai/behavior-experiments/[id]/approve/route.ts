...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      body?.confirmation !== 'APPROVE BEHAVIOR VERSION'
    ) {
      return NextResponse.json(
        { error: 'Promotion requires explicit production approval' },
        { status: 409 }
      );
    }
    const { id } = await context.params;
    await approveAiBehaviorExperiment(supabaseAdmin(), accountId, id);
